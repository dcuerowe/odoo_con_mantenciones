...
**Ejecución al 2026-05-28:** 78 pruebas verdes (L1+L2 reejecutadas el 2026-05-28; L3 de la corrida
del 2026-05-27). Evidencia reproducible y vinculación con los objetos reales del test-Odoo en
[`RESULTADOS.md`](../RESULTADOS.md).

- **L1 unitario (12):** `ordenar_respuestas` (8) + `check_new_sub` (4).
- **L2 componente (47), todas las ramas por módulo con OdooSpy:** MC 13, CF 9, I 9,
  MP 9, R 7. Incluye el testigo de **OBS-11** (serial float sin normalizar en la búsqueda).
  Cubren: S/N no encontrado (punto inexistente / con transferencia / sin
...
| OBS-11 | **Punto único de falla.** La búsqueda exacta del serial no se normaliza: el bloque float→int (`processor.py:340-342` en MC; equivalente por módulo) corre **tras** capturar  `serial_X` y muta el dict, no la variable usada en `search_read('maintenance.equipment', [['serial_no','=',serial_X]])` (MC L363, CF L990, R L1749). Un serial numérico que pandas infiere `float` (`24000.0`) se busca contra el char `"24000"` → no calza → equipo no hallado → cae a "S/N no encontrado" y **no crea/actualiza la solicitud**. Sin `.strip()` ni cast a string. | `test_mc_serial_float_no_se_normaliza_obs11` | **Alta**  |

> Estas observaciones provienen de la lectura del código y de [processor_documentation](../../flows/processor_documentation.md). Antes de "arreglarlas", confirmar con negocio: algunas pueden ser comportamiento intencional.
