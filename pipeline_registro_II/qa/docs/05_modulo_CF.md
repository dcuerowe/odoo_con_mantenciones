...
**Algoritmo de selección (CF):** Prioridad 1 = solicitud en `stage=3`. Prioridad 2 = la de `schedule_date` más cercana a `fecha`. **Efecto colateral:** archiva las de fecha anterior a la seleccionada.
...
> **Riesgo R6 (selección/archivado):** TC-CF-04 es el caso de mayor valor — verifica que se actualice la solicitud correcta **y** que el archivado no toque la elegida ni las posteriores. El oráculo debe afirmar el conjunto exacto de IDs archivados.
